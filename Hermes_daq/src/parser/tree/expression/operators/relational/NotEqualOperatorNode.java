...
public class NotEqualOperatorNode extends BinaryOperator implements RelationalOperator {
    ...
    public Type evaluateSemantic() throws SemanticException {
        ...
        String errorMessage = getSemanticErrorMessage("Not Equal operator", left, right);
        throw new SemanticException(errorMessage);
    }

    private boolean operationIsValid(Type left, Type right){
        return (isSetType(left)&&isSetType(right))
                || (isIntType(left)&&isIntType(right))
